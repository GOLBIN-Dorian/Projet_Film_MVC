...
function createFilm() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errors)) {
            $createFilmSuccess = createFilmData($_POST);

            if ($createFilmSuccess != false) {
                setSuccessMessage("Film créé avec succès");
                header("Location: index.php");
                exit;
            } else {
                setErrorMessage("Erreur lors de la création du film");
            }
        } else {
            $film = $_POST;
        }
...
/*     if ($_SERVER['REQUEST_METHOD'] === 'POST' && validateFilmData($_POST) === []) {
        createFilmData($_POST);
        header("Location: index.php?action=index");
        exit;
    
    } else if ($_SERVER['REQUEST_METHOD'] === "POST" && validateFilmData($_POST) != []) {
        $errors =  validateFilmData($_POST);
